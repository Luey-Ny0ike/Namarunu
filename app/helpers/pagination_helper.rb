...
module PaginationHelper
  ...
  def link_to_next_page(namespace, page, activate_when_observed: false,
                        label: default_pagination_label(activate_when_observed), data: {}, **attributes)
    return unless page.before_last? && !params[:previous]
  ...
  def pagination_link(namespace, frame_page_number:, page_param:, activate_when_observed: false,
                      label: default_pagination_label(activate_when_observed), url_params: {}, data: {}, **attributes)
  ...
  def default_pagination_label(_activate_when_observed)
    "Load more…"
  end
end
